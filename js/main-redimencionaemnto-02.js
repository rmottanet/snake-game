...
function resizeCanvas() {
    // Considere qualquer margem, borda ou preenchimento que você tenha em outros elementos
    const bodyStyles = window.getComputedStyle(document.body);
    const bodyPaddingTop = parseInt(bodyStyles.paddingTop);
    const bodyPaddingRight = parseInt(bodyStyles.paddingRight);
    const bodyPaddingBottom = parseInt(bodyStyles.paddingBottom);
    const bodyPaddingLeft = parseInt(bodyStyles.paddingLeft);

    // Ajuste o tamanho do canvas para corresponder às dimensões da janela, considerando margens, bordas ou preenchimentos
    canvas.width = window.innerWidth - bodyPaddingLeft - bodyPaddingRight;
    canvas.height = window.innerHeight - bodyPaddingTop - bodyPaddingBottom;
}
